 ...
 year = [
  ...
  ngOnInit(): void {
    ...
    this.registrationService.getDefaultProfileImage().subscribe(response => {
      this.url = "data:image/jpeg;base64," + response.image;
    }, err => {
  ...
  connectNewAccount() {
    ...
    request.username = this.linkedAccount.username;
    request.firstname = this.linkedAccount.firstname;
    request.lastname = this.linkedAccount.lastname;
    request.relationToMainAccount = this.linkedAccount.relation;
    request.address = this.linkedAccount.address;
    request.birthDate = new Date(this.linkedAccount.year['name'], this.linkedAccount.month['id'], this.linkedAccount.day['id']);
    request.city = this.linkedAccount.city;
    request.country = this.linkedAccount.country;
    request.gender = this.linkedAccount.gender;
    request.postCode = this.linkedAccount.postCode;

    if (this.profileImage) {
      request.profileImage = this.profileImage;
    }
    
    this.linkedAccountsService.connectNewAccount(request).subscribe(response => {
      console.log(response);
      this.router.navigateByUrl('/profile/create1/' + response.id);
    }, err => {
      console.log(err);
    });
  }
